...
function afficheLeProduit($id){
    ...
    if ($monObjPdoStatement->execute()) {
        ...
        $monObjPdoStatement=$pdo->prepare("INSERT INTO medecinproduit(idMedecin, idProduit, Date, Heure) "
                . "VALUES(?,?,?,?)");
        $monObjPdoStatement->execute(array($_SESSION['id'],$id,date('d-m-y'),date('h:i:s')));
        return $leProduit;
    }
    else
        throw new Exception("erreur");
...
function afficheLaVisio($id){
    ...
    if ($monObjPdoStatement->execute()) {
        ...
        $monObjPdoStatement=$pdo->prepare("INSERT INTO medecinvisio(idMedecin, idVisio, dateInscription) "
                . "VALUES(?,?,?)");
        $monObjPdoStatement->execute(array($_SESSION['id'],$id,date("y-m-d H:i:s")));
        return $laVisio;
    }
    else
        throw new Exception("erreur");
