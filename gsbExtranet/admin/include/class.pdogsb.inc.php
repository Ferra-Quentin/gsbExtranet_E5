...
function supprimerProduit($id){
    $pdo = PdoGsb::$monPdo->prepare("DELETE FROM medecinproduit WHERE idProduit = ?");
...
function supprimerVisio($id){
    $pdo = PdoGsb::$monPdo->prepare("DELETE FROM medecinvisio WHERE idVisio = ?");
    $pdo->execute(array($id));
...
function archiverMedecin($id){
    ...
    $pdoStement= $pdo->prepare("INSERT INTO archivagemedecin(id_medecin,anneNaissance,dateCreation) VALUES(null,?,?)");
